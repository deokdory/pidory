...
- Discord Bot Token
- Linux 또는 macOS
- PostgreSQL 17 (권장) 또는 시스템 기본 버전 — Linux에서는 `scripts/postgres-setup.sh`가 자동 설치
...
**서비스 배포 (권장 — Linux systemd):**

```bash
./deploy/install.sh                  # 바이너리 빌드, systemd 서비스 설치, 스킬 설치
sudo bash scripts/postgres-setup.sh # PostgreSQL 설치, DB 생성, /etc/pidory/db.env 작성, 서비스 재시작
```

이걸로 끝. 서비스가 자동으로 시작되고 `/etc/pidory/db.env`의 `DATABASE_URL`로 PostgreSQL에 연결해.

**수동 / 개발 환경 (서비스 없이):**

```bash
# PostgreSQL 인스턴스를 가리키는 DATABASE_URL 설정
...
```bash
./deploy/install.sh                  # 바이너리 빌드, 서비스 설치, 스킬 설치
sudo bash scripts/postgres-setup.sh # PostgreSQL 설정 및 서비스 시작
sudo systemctl status pidory
journalctl -u pidory.service -f
```

`install.sh`는 릴리즈 바이너리를 빌드하고, config가 없으면 `config.toml.example`을 복사하며, 서비스 파일을 설치하고 부팅 시 자동 시작을 등록하고, `pidory-migrate` 마이그레이션 바이너리를 `/usr/local/bin/`에 설치하며, 내장 스킬도 `~/.claude/skills/`에 자동 배포해.

`postgres-setup.sh`는 PostgreSQL 17을 설치하고 (없으면 시스템 기본 버전으로 폴백), `pidory` role과 데이터베이스를 생성하고, `DATABASE_URL`을 `/etc/pidory/db.env` (mode 600)에 쓰고, 서비스를 재시작해.
...
`install.sh`는 릴리즈 바이너리를 빌드하고, config가 없으면 `config.toml.example`을 복사하며, 서비스 파일을 설치하고 부팅 시 자동 시작을 등록해. 내장 스킬도 `~/.claude/skills/`에 자동 배포해. 참고: `postgres-setup.sh`는 Linux/systemd 전용이야. macOS에서는 실행 전에 `DATABASE_URL`을 수동으로 설정해야 해.

## 데이터베이스 (PostgreSQL)

pidory는 PostgreSQL을 데이터베이스 백엔드로 사용해. 연결은 `DATABASE_URL` 환경변수로 설정하며, **이 값이 권위 있는 설정 소스야**. `config.toml`의 `[database] path` 필드는 deprecated이며 런타임에 무시돼.

### DATABASE_URL

Linux 서비스 배포 시 `DATABASE_URL`은 `/etc/pidory/db.env`에서 주입돼 (systemd `EnvironmentFile` 경유). 이 파일은 `scripts/postgres-setup.sh`가 작성하며 mode 600이야 (pidory 서비스 사용자만 읽을 수 있음).
...
수동 또는 개발 환경에서는 실행 전 변수를 export해:
...
`PIDORY_LEGACY_DB` 환경변수로 레거시 SQLite 경로를 override할 수 있어 (기본: `/var/lib/pidory/pidory.db`). 이건 마이그레이션 바이너리(`pidory-migrate`)에만 관련 있어. 첫 실행 시 SQLite 소스를 읽어 PostgreSQL로 데이터를 가져와.

### 자동 마이그레이션

pidory는 2단계 마이그레이션 안전망을 사용해:

1. **`pidory-migrate` ExecStartPre** — 서비스 시작 전에 실행돼. PostgreSQL DB가 비어 있으면 SQLite 소스에서 트랜잭션 단위로 데이터를 가져와. 멱등: 이후 실행은 no-op.
2. **`init_pool`의 `sqlx::migrate!`** — 시작마다 미적용 스키마 마이그레이션을 적용해. 버전 업그레이드 자동 처리.

마이그레이션은 **roll-forward only**야. PostgreSQL에서 SQLite로 되돌리는 도구는 없어.

### PostgreSQL 버전

PostgreSQL 17을 권장해. `scripts/postgres-setup.sh`가 apt로 `postgresql-17`을 설치하고, 없으면 시스템 기본 `postgresql` 패키지로 폴백해. 다른 버전(14, 15, 16)도 동작해.

### 알려진 제한 사항

`update/backup.rs`의 `restore_db()` (self-update 자동 롤백 시 DB 복원)는 `sqlite3` CLI를 사용해서 PostgreSQL 환경에서는 동작하지 않아. 일반 동작에는 영향 없고, 실패한 self-update 후 수동으로 `pg_dump` / `psql`로 복원하면 돼. 후속 PR에서 `pg_dump` 기반으로 전환 예정.
...
> **Deprecated.** `DATABASE_URL` 환경변수가 데이터베이스 설정의 권위 있는 소스야. 아래 `path` 필드는 하위 호환성을 위해 유지되며 런타임에 무시돼.

| 필드 | 설명 | 기본값 |
|------|------|--------|
| `path` | ~~SQLite 데이터베이스 파일 경로~~ (deprecated, 무시됨) | `"pidory.db"` |

`DATABASE_URL`은 `/etc/pidory/db.env` (Linux 서비스) 또는 환경변수 (수동/개발)로 설정해.
