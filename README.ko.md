...
[![버전](https://img.shields.io/badge/version-v0.6.7-green.svg)](https://github.com/deokdory/pidory/releases)
...
- **`/update` 사전 검증** — 최신 릴리스를 가져오고 체크섬을 검증한 후 바이너리를 교체
...
```bash
sudo bash scripts/postgres-setup.sh   # PostgreSQL 설치, DB 생성, /etc/pidory/db.env 작성
bash deploy/install.sh                # 바이너리 빌드, systemd 서비스 설치, 스킬 배포
```
...
| `/update` | 최신 릴리스를 가져와 검증 후 바이너리 교체 | 소유자 전용 |
...
> **주의: 멀티유저 베타 — Always Allow는 스레드 내 모든 사용자에게 적용돼요.**
>
...
`/update` 슬래시 커맨드(소유자 전용)로 안내에 따라 자체 업데이트를 수행해요:

1. GitHub에서 최신 릴리스 태그 조회
2. 릴리스 바이너리 다운로드 및 체크섬 검증
3. 실행 중인 바이너리를 교체하고 서비스 재시작
...
| `subprocess/parser.rs` | JSON 라인 → `StreamEvent` 파싱 (Init, Assistant, User, RateLimit, Result, ControlRequest 등) |
| `subprocess/permission.rs` | `PermissionCache` (세션별 Always Allow 집합); oneshot 채널을 통한 `PermissionRequest`/`PermissionDecision` |
| `handler/message.rs` | Discord 이벤트를 세션 큐로 라우팅; 500ms fast-complete 판정; Discord에 실시간 스트리밍 |
