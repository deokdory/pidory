...
echo ""
echo "Next steps:"
echo "  1. token inject — deok-guard 키 둘 중 살아있는 것:"
echo "     deok-guard inject $PROJECT_DIR/.env.qa --env-line 'PIDORY_DEV_DISCORD_TOKEN=secret:PIDORY_DEV_DISCORD_TOKEN'"
echo "     # 또는: --env-line 'PIDORY_DEV_DISCORD_TOKEN=secret:pidory-dev/discord-token'"
echo "  2. config.qa.toml 검토 — guild_id 본인 qa server인지"
echo "  3. migrate-enabled branch를 qa worktree에 deploy (pidory-migrate binary 빌드):"
echo "     bash $SOURCE_DIR/scripts/qa-deploy.sh 304-postgres-migration"
echo "  4. postgres + qa db 셋업 (봇 자동 시작):"
